...
Application of the methods described herein may be covered by U.S. Patent.  \cite{pav_nmf}
}}
...
test_a_bunch <- function(Y_t, L_0, R_0, niter=1e4L) {
	iter_history <<- rep(NA_real_, niter)
	on_iteration_end <- function(iteration, Y, L, R, ...) {
		iter_history[iteration] <<- frobenius_err(Y,L,R)
	}
	wuz <- aurnmf(Y_t, L_0, R_0, max_iterations=length(iter_history), on_iteration_end=on_iteration_end)
	df1 <- tibble(x=seq_along(iter_history),y=iter_history) %>% mutate(method='additive, optimal step')

	#iter_history <<- rep(NA_real_, niter)
	#wuz <- aurnmf(Y_t, L_0, R_0, max_iterations=length(iter_history), check_optimal_step=FALSE, on_iteration_end=on_iteration_end)
	#df15 <- tibble(x=seq_along(iter_history),y=iter_history) %>% mutate(method='additive, naive step')

	#iter_history <<- rep(NA_real_, niter)
	#wuz <- aurnmf(Y_t, L_0, R_0, max_iterations=length(iter_history), check_optimal_step=FALSE, tau=0.9, on_iteration_end=on_iteration_end)
	#df17 <- tibble(x=seq_along(iter_history),y=iter_history) %>% mutate(method='additive, naive step, tau=0.9')

	iter_history <<- rep(NA_real_, niter)
	wuz <- murnmf(Y_t, L_0, R_0, max_iterations=length(iter_history), on_iteration_end=on_iteration_end)
	df2 <- tibble(x=seq_along(iter_history),y=iter_history) %>% mutate(method='multiplicative')
...
The multiplicative and additive algorithms are implemented in the R package \textsc{rnmf} written by the author.  \cite{rnmfrnmf-Manual}
